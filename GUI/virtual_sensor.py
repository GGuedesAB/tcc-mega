...
class virtual_serial ():
    ...
    def Serial (port, baudrate):
        return virtual_serial(port, baudrate)

    def read_until(self, byte_string):
        return_str=""
        for i in range(16):
            value=random.randint(880,1023)
            return_str+="{:04d}".format(value)+"|"
        return_str+="\r\n"
...
class Oscilloscope ():
    def __init__ (self, config):
        ...
        self.sample = 0
        self.ser = virtual_serial.Serial(port=self.port, baudrate=self.baud)
        self.connected=True
...
def consume_reading(measurement_queue, num_sensors):
    ...
    while True:
        try:
            measurement_buffer = measurement_queue.get(block=True)
